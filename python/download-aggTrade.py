...
def download_monthly_aggTrades(type, symbols, num_symbols, years, months, start_date, end_date, folder, checksum):
  current = 0
  date_range = None
    ...
    for year in years:
      for month in months:
        ...
        if current_date >= start_date and current_date <= end_date:
          path = "data/{}/monthly/aggTrades/{}/".format(type, symbol.upper())
          file_name = "{}-aggTrades-{}-{}.zip".format(symbol.upper(), year, '{:02d}'.format(month))
          download_file(path, file_name, date_range, folder)

          if checksum == 1:
            checksum_path = "data/{}/monthly/aggTrades/{}/".format(type, symbol.upper())
            checksum_file_name = "{}-aggTrades-{}-{}.zip.CHECKSUM".format(symbol.upper(), year, '{:02d}'.format(month))
            download_file(checksum_path, checksum_file_name, date_range, folder)
    
    current += 1

def download_daily_aggTrades(type, symbols, num_symbols, dates, start_date, end_date, folder, checksum):
  current = 0
  date_range = None
  ...
  for symbol in symbols:
    ...
    for date in dates:
      ...
      if current_date >= start_date and current_date <= end_date:
        path = "data/{}/daily/aggTrades/{}/".format(type, symbol.upper())
        file_name = "{}-aggTrades-{}.zip".format(symbol.upper(), date)
        download_file(path, file_name, date_range, folder)

        if checksum == 1:
          checksum_path = "data/{}/daily/aggTrades/{}/".format(type, symbol.upper())
